...
这里也要讲清楚：这还不等于“Deepvan 从 2024 年到现在的全量内容”。现在证明的是，当前已经发现的主页/回答/文章/想法主 URL 已经补齐。主页长滚动这轮拿到 227 个 pin URL，其中 222 条确认是 Deep Van 本人公开想法，2 条打开后是“荒原/资源不存在”，3 条是页面里混入的其他作者 pin，已经从语料里剔除。文章页显示总文章数高于当前稳定捕获到的 41 条文章，发现层还要继续往更早处补。后面老内容只保留正文和 OCR 文本，不长期存图片。

本轮还修了两个容易把数据搞脏的问题：

- 只取每条主页列表项的主 URL，不把“上期回答”等文内链接重复算进缺口。
- Activity 里的“赞同了回答/文章”不算 Deepvan 本人原文，只能作为发现线索。
- 想法长滚动里出现的他人 pin 不再自动算作 Deep Van 原创，必须逐条打开确认作者。
- 公开看不到、已删除、设私密或资源不存在的内容，只进入不可访问清单，不进入 corpus 和评分。

组合层也有了第一版结果：国际版 / 全球版已经能从 OCR 表里还原出多期完整明细，最新一张高置信表是 2026-03-15，合计约 100%；内地版 / 公募基金版现在先按模块还原，2026-06-29 的结构是 `QDII 主线组合 75% + 黄金 10% + 国金量化多因子 10% + 大成动态量化配置 5%`。文本里的调仓句子也抽出来了，但这部分先作为候选事件，真正回测优先吃完整组合表。
