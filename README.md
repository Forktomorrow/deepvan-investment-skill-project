...
截至 2026-07-10，本地先做了一轮“数据源体检”。现在的原则很简单：只要是已经在主页、回答、文章、想法列表里发现的 Deepvan 本人主 URL，就必须有高置信原文缓存；赞同别人的回答、文内“上期回答”链接、第三方总结，不再混进本人原文覆盖率。
...
| 有效 OCR 原文 | 53 篇 |
| OCR 文本 | 约 5.9 万字 |
| 当前高置信缓存发布时间范围 | 2024-07-23 到 2026-07-10 |

为什么“全部高置信原文”比“已发现本人主 URL”多？因为缓存里还有搜索、旧候选和手工核验时抓到的额外原文。它们可以进入语料池，但不拿来计算“主页发现覆盖率”。

这里也要讲清楚：这还不等于“Deepvan 从 2024 年到现在的全量内容”。现在证明的是，当前已经发现的主页/回答/文章/想法主 URL 已经补齐。文章页显示总文章数为 47，当前稳定捕获到的文章列表为 41 条，说明发现层还要继续往更早处补。后面老内容只保留正文和 OCR 文本，不长期存图片。

本轮还修了两个容易把数据搞脏的问题：

- 只取每条主页列表项的主 URL，不把“上期回答”等文内链接重复算进缺口。
- Activity 里的“赞同了回答/文章”不算 Deepvan 本人原文，只能作为发现线索。
...
这一步是 skill 变扎实的基础。否则只靠高热或高关键词内容，容易把他的思想结构抽偏。当前已发现内容已经补齐，下一步重点不是“补缓存”，而是“继续发现更早的本人原文”。
